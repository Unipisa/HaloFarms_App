...
fun HaloFarmsApp() {
    HaloFarmsTheme {
        Surface(color = Color.White) {
            ...
            // Repositories (room)
            val pointValueRepo = PointValueRepo(HaloFarmsDatabase.getInstance(context).pointValueDao())
            val mapRepo = MapRepo(HaloFarmsDatabase.getInstance(context).mapDao())
            val perimeterRepo = PerimeterPointRepo(HaloFarmsDatabase.getInstance(context).perimeterPointDao())
            val sampleRepo = SampleRepo(HaloFarmsDatabase.getInstance(context).sampleDao())
