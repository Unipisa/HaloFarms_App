...
class FirestoreProxy {
     ...
     *
     * @return db The Firestore instance of the default FirebaseApp
     */
    @Singleton
    ...
    fun delDocReference(collection: String, reference: List<Any>) {
        for (ref in reference) {
            ...
            db().collection("$username-${collection}").document(ref.toString())
                .delete()
    ...
    fun delPoints(mapName: String) {
        ...
        val querySamples = db().collection("$username-samples").whereEqualTo("zoneName", mapName)
        querySamples.get()
            .addOnSuccessListener { documents ->
                for (document in documents) {
                    document.reference
        ...
        val queryPerimeterPoint = db().collection("$username-perimeterPoints").whereEqualTo("zoneName", mapName)
    ...
    fun addPerimeterPoint(point: PerimeterPoint) {
        ...
        // Sends the map to the Firestore
        db().collection("$username-perimeterPoints").document("${point.latitude},${point.longitude}").set(p)
            .addOnSuccessListener {
    ...
    fun addSample(sample: Sample) {
        ...
        // Sends the map to the Firestore
        db().collection("$username-samples").document("${sample.latitude},${sample.longitude}").set(p)
            .addOnSuccessListener {
    ...
    fun addPoint(point: PointValue) {
        ...
        // Sends the map to the Firestore
        db().collection("$username-points").document("${point.latitude},${point.longitude}").set(p)
            .addOnSuccessListener {
    ...
    fun updateToBeAnalyzed(latitude: Double, longitude: Double, toBeAnalyzed: Boolean) {
        val pointRef = db().collection("$username-samples").document("${latitude},${longitude}")
...
/*
    fun updateSar(latitude: Double, longitude: Double, newSar: Double) {
        val pointRef = db().collection("$username-points").document("${latitude},${longitude}")
    ...
    *//**
    ...
    fun updateArea(mapName: String, newArea: Double) {
        val pointRef = db().collection("$username-maps").document(mapName)
    ...
    fun updateLatLng(mapName: String, latitude: Double, longitude: Double) {
        val mapRef = db().collection("$username-maps").document(mapName)
    ...
    val smpls: Flow<List<Sample>>
        get() = db().collection("$username-samples").snapshots().map { snapshot ->
    ...
    fun FromCloud() {
        ...
        val pPointRepo = PerimeterPointRepo(db.perimeterPointDao())
        val sampleRepo = SampleRepo(db.sampleDao())
        //Maps
        db().collection("$username-maps").get()
        ...
        db().collection("$username-points").get()
            .addOnSuccessListener { result -> pointsValueFromCloud(pointValueRepo, result.toObjects(PointValue::class.java)) }
            .addOnFailureListener {  toaster(CoroutineScope(Dispatchers.Main), context, R.string.you_need_internet_for_your_first_run) }

        db().collection("$username-samples").get()
            .addOnSuccessListener { result -> samplesFromCloud(sampleRepo, result.toObjects(Sample::class.java)) }
            .addOnFailureListener {  toaster(CoroutineScope(Dispatchers.Main), context, R.string.you_need_internet_for_your_first_run) }

        db().collection("$username-perimeterPoints").get()
            .addOnSuccessListener { result -> pPointsFromCloud(pPointRepo, result.toObjects(PerimeterPoint::class.java)) }
            .addOnFailureListener {  toaster(CoroutineScope(Dispatchers.Main), context, R.string.you_need_internet_for_your_first_run) }
